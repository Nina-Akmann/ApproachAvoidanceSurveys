...
function openSocialShare(url) {
    var newWnd = window.open(url, "ShareSocial", 'toolbar=0,menubar=0,resizable=1,top=300,left=300,width=550,height=600');
    newWnd.opener = null;
}
...
function resetMenu(className) {
    ...
    if ($j('.mm-container').hasClass('mm-menu-expanded')) {
        $j('.mm-container').removeClass('mm-menu-expanded');
        $j('.topnav').hide();
    }

    if (!$j('#srchForm').is(':hidden')) {
        $j('#srchForm').hide();
    }    
}
...
function resetHeroHeight() {
    ...
    if (hero.length > 0) {
        ...
        if ($j('.trapezoid').length == 0) {
            ...
            if (Modernizr.mq('(max-width: 995px)')) wrapHeight = (imgHeight + 60) + "px";
        }
...
function setNavActiveElt() {
    ...
    var elt = (jQuery('.subBrandNav li a[href="' + url + '/' + '"]').length != 0 ?
        ...
        if ($j(this).find('img').length == 0) {
            ...
            if (typeof aHref != "undefined") {
                ...
                if (isSecure) {
                    ...
                    if (aHref.indexOf('/') == 0 || aHref.indexOf('.apa.org') > 0 || aHref.indexOf('.apadivisions.org') > 0 || aHref.indexOf('.apaservices.org') > 0 || aHref.indexOf('.supportpsychologypac.org') > 0) {
...
function toggleCitation() {
    isSelected = false;
    var popup = document.getElementById("citationPopup");
    if (popup.classList.contains("showCitationPopup")) {
        hideMask("rwdOverlayMask", "citationPopup");
    } else {
        showMask("rwdOverlayMask");
        showCitationPopup();
    }
...
// Detect any click outside citation body
$j(document).click(function (e) {
    var target = e.target;

    if (!$j(target).is("#citationBody") && !$j(target).is("#citationBody i")) {
        var cBody = document.getElementById("citationBody");
        isSelected = false;
        if (cBody != null) {
            cBody.classList.remove("citationBodySelected");
            cBody.classList.add("citationBody");
        }
    }
});
...
function selectAll() {
    var cBody = document.getElementById("citationBody");
    cBody.classList.remove("citationBody");
    cBody.classList.add("citationBodySelected");
    if (window.getSelection) {
        var range = document.createRange();
        range.selectNodeContents(cBody);
